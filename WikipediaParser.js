...
module.exports =
{
    fetchArticleElements: function(articleTitle)
    {
        return new Promise(function(resolve, reject)
        {
            ...
            request({url: 'https://es.wikipedia.org/w/api.php?action=query&titles='  + articleTitle + '&utf8=1&&prop=pageimages&format=json&pithumbsize=500&redirects=', forever: true} , function (error, response, imageBody)
            {
                ...
                request({url: 'https://es.wikipedia.org/?curid='+ idWiki + '?action=render', forever: true}, function (error, response, wikiBody)
                {
                    if (error)
                    {
