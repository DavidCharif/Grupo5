...
import {insertar_registro, consultar_tabla, conexion} from './db.js';
...
app.get('/agregar/:nombre/:apellido/:telefono/:correo/:contrasena', function(peticion, respuesta)
{
...
app.get("/login", function (petition,respuesta){
    ...
    respuesta.sendFile(dirForm + "/login.html");
});
...
app.post('/auth', function(request, response) {
	...
	if (username && password) {
		connection.query('SELECT * FROM Usuarios WHERE Correo = ? AND Contraseña = ?', [username, password], function(error, results, fields) {
		    ...
		    /* 
		    connection.query('SELECT Nombre FROM Usuarios WHERE Correo = ?', [correo], function(error, results) {
		      nombre = results;
